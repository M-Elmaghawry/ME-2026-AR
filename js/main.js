...
function initTypewriter() {
    const texts = [
        "منسق BIM",
        "مهندس BIM الإنشائي",
        "مهندس BIM للبنية التحتية",
        "مهندس محترف معتمد - هيئة المهندسين السعوديين",
        "مدرب معتمد من أوتوديسك",
        "محترف معتمد في Revit الإنشائي",
        "محترف معتمد في Revit المعماري"
    ];
...
if (contactForm) {
    contactForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showFormStatus('شكرًا على رسالتك! سنتواصل معك قريبًا.', 'success');
            contactForm.reset();
            hideFormStatus();
        } catch (error) {
            console.error('EmailJS send error:', error);
            showFormStatus('عذرًا، تعذّر إرسال رسالتك. يُرجى المحاولة مرة أخرى.', 'error');
            hideFormStatus(7000);
        } finally {
